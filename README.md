...
## arguments
   ...
   4. A function that exits with no error when the first argument `"$1"` is the 'default' item, which should be treated as the root of the `xioxide` item tree. For example, a natural value for this when using `xioxide` for directories would be `$HOME`. So for this field, you could pass a string like this: `[ "$1" == "$HOME" ] && return || return 1`.

   5. The stem name of the configuration file to use (defaults to `default` if left empty). So if you want to use the configuration `$XDG_CONFIG_HOME/xioxide/my.conf`, pass `my` as this argument. (Also affects the sed filter file used, see [search patterns section](#search-pattern))

   6. The search pattern to search through the filtered list and select final output. More on the search pattern and how it is parsed [later](#search-pattern)
...
## search pattern
   ...
   Sed Filter:
   `xioxide` provides a convenient option where you can modify the letters of the search string before it is processed (the letters being the characters from the first non-dot char to the last non-dot char, inclusive). Next to your config files in `$XDG_CONFIG_HOME/xioxide/<<stem name>>.sed` you can put a sed script that will be run on the letters if the search is occurring from the root of the tree (so either there is a single pre-dot, or the current item returns without error when passed through the default item function (see [arguments section](#arguments))). If you do not create this file, the search string will not be modified. This is useful if you have some items that you access frequently but they have a long prefix, you can use this to 'alias' them.
